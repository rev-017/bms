...
if(isset($_POST['btn_add'])){
    ...
    $txt_zone = $_POST['txt_zone'];
	 $txt_hNoStreet = $_POST['txt_hNoStreet'];
    ...
    $su = mysqli_query($con,"SELECT * from resident where username = '".$txt_uname."' ");
    ...
    if($ct == 0){
        ...
        if($name != ""){
            if(($imagetype=="image/jpeg" || $imagetype=="image/png" || $imagetype=="image/bmp") && $size<=2048000){
                    ...
                    {
                    $txt_image = $image;
                    $query = mysqli_query($con,"CALL sp_resident_add
                                    (
                                        '$txt_lname', 
                                        '$txt_fname', 
                                        '$txt_mname',  
...
if(isset($_POST['btn_save']))
{
    ...
    $txt_edit_hNoStreet = $_POST['txt_edit_hNoStreet'];
...
$su = mysqli_query($con,"SELECT * from resident where username = '".$txt_edit_uname."' and id not in (".$txt_id.") ");
$ct = mysqli_num_rows($su);
...
if($ct == 0){
    ...
    if($name != ""){
            if(($imagetype=="image/jpeg" || $imagetype=="image/png" || $imagetype=="image/bmp") && $size<=2048000){
                ...
                $update_query = mysqli_query($con,"CALL sp_resident_update
                                        ('$txt_edit_lname',
                                       '$txt_edit_fname',
                                        '$txt_edit_mname.',
                                        '$txt_edit_bdate',
        ...
        $chk_image = mysqli_query($con,"SELECT * from resident where id = '".$_POST['hidden_id']."' ");
...
if(isset($_POST['btn_delete']))
{
    if(isset($_POST['chk_delete']))
    {
        ...
        {
            $delete_query = mysqli_query($con,"DELETE from resident where id = '$value' ") or die('Error: ' . mysqli_error($con));
